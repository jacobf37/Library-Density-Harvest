...
namespace Landis.Library.DensitySiteHarvest
{
        ...
        : Landis.TextParser<T>
    {
        ...
        //private DensityCohortSelectors cohortSelector;
        //private DensityCohortSelectors additionalCohortSelector;

        private DensityCohortSelectors densitySelector;
        private DensityCohortSelectors additionalDensitySelector;
        ...
        /// <summary>
        /// Reads a list of species and diameter classes eligible for cutting.
        /// </summary>
        protected ICohortSelector ReadSpeciesAndDiameters(params string[] names)
        {
            ...
            if (names.Length == 3)
            {
                //JSF - FIX for additional selector
                additionalDensitySelector = new DensityCohortSelectors();
            }
            else
            {
                densitySelector = new DensityCohortSelectors();
            }
            ...
            while (!AtEndOfInput && !namesThatFollow.Contains(CurrentName))
            {
                ...
                //  Read one or more diameters or diameter ranges
                List<float> diameters = new List<float>();
                List<DiameterRange> ranges = new List<DiameterRange>();
                currentLine = new StringReader(CurrentLine.Substring(indexOfDataAfterSpecies));
                InputVar<DiameterRange> diameterOrRange = new InputVar<DiameterRange>("Diameter or Diameter Range");
                while (currentLine.Peek() != -1)
                {
                    ReadValue(diameterOrRange, currentLine);
                    ValidateDiameterOrRange(diameterOrRange.Value, diameters, ranges);
                    TextReader.SkipWhitespace(currentLine);
                }
            ...
            if (names.Length == 3)
            {
                return additionalDensitySelector;
            }
            else
            {
...
/*        //---------------------------------------------------------------------
        ...
        protected void ValidateDiameterOrRange(InputValue<DiameterRange> diameterOrRange,
                                          List<float> diameters,
                                          List<DiameterRange> ranges)
        {
            if (diameterOrRange.String.Contains("-"))
            {
                DiameterRange range = diameterOrRange.Actual;

                //  Does the range contain any individual ages?
                foreach (float diameter in diameters)
                {
                    if (range.Contains(diameter))
                        throw new InputValueException(diameterOrRange.String,
                                                      "The range {0} contains the age {1}",
                                                      diameterOrRange.String, diameter);
                }

                //  Does the range overlap any previous ranges?
                foreach (DiameterRange previousRange in ranges)
                {
                    if (range.Overlaps(previousRange))
                        throw new InputValueException(diameterOrRange.String,
                                                      "The range {0} overlaps the range {1}-{2}",
                                                      diameterOrRange.String, previousRange.Start, previousRange.End);
                }
            ...
            {
                float diameter = diameterOrRange.Actual.Start;

                //  Does the age match any of the previous ages?
                foreach (float previousDiameter in diameters)
                {
                    if (diameter == previousDiameter)
                        throw new InputValueException(diameterOrRange.String,
                                                      "The diameter {0} appears more than once",
                                                      diameterOrRange.String);
                }

                //  Is the age in any of the previous ranges?
                foreach (DiameterRange previousRange in ranges)
                {
                    if (previousRange.Contains(diameter))
                        throw new InputValueException(diameterOrRange.String,
                                                      "The diameter {0} lies within the range {1}-{2}",
                                                      diameterOrRange.String, previousRange.Start, previousRange.End);
                }
        ...
        protected Planting.SpeciesList ReadSpeciesToPlant()
        {
            ...
            InputVar<Dictionary<ISpecies, uint>> plant = new InputVar<Dictionary<ISpecies, uint>>(ParameterNames.Plant, ReadSpeciesList);
        ...
        /// </summary>
        public InputValue<Dictionary<ISpecies, uint>> ReadSpeciesList(StringReader currentLine,
                                                          out int index)
        {
